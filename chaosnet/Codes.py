...
def k_cross_validation(FOLD_NO, traindata, trainlabel, testdata, testlabel, INITIAL_NEURAL_ACTIVITY, DISCRIMINATION_THRESHOLD, EPSILON):
    ...
    for DT in DISCRIMINATION_THRESHOLD:
        ...
        for INA in INITIAL_NEURAL_ACTIVITY:
            ...
            for EPSILON_1 in EPSILON:
                ...
                print("f1 for Q = ", Q[ROW, COL, WIDTH ],"B = ", B[ROW, COL, WIDTH ],"EPSILON = ", EPS[ROW, COL, WIDTH ]," is  = ",  np.mean(FSCORE_TEMP)  )
    ...
    np.save(RESULT_PATH+"/h_fscore.npy", MAX_FSCORE )    
    np.save(RESULT_PATH+"/h_Q.npy", Q_MAX ) 
    np.save(RESULT_PATH+"/h_B.npy", B_MAX )
    np.save(RESULT_PATH+"/h_EPS.npy", EPSILON_MAX )
