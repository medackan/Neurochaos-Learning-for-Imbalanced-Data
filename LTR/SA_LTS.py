...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = DecisionTreeClassifier(min_samples_leaf = 5, random_state=42, max_depth = 5, ccp_alpha = 0.0)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = SVC(C = 4.4, kernel='rbf', decision_function_shape='ovr', random_state = 42)
        clf.fit(X_train, y_train.ravel())
        Y_PRED = clf.predict(X_test)
        F1SCORE = f1_score(y_test, Y_PRED, average='macro')
        final_f1_score.append(F1SCORE)  
